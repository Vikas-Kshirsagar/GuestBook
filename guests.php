...
//error_reporting(0);
 ...
 {
   $rows = mysql_query("SELECT * FROM guests where users_id='".$_SESSION['GuestBook']['id']."' AND name like '%".$_POST['search_text']."%' ORDER BY name LIMIT $start_from, 5");
    $numrows = mysql_num_rows(mysql_query("SELECT * FROM guests where users_id='".$_SESSION['GuestBook']['id']."' AND name like '%".$_POST['search_text']."%'"));
 }
 else
 {
   $rows = mysql_query("SELECT * FROM guests where users_id='".$_SESSION['GuestBook']['id']."' ORDER BY name");
   $numrows = mysql_num_rows(mysql_query("SELECT * FROM guests where users_id='".$_SESSION['GuestBook']['id']."'"));
 }
$countrow = mysql_fetch_array($rows);
 	...
 	<div class="inner">
	    ...
	    <div class="actions-bar wat-cf">
                <form class="form" method="post">
					...
					<span><input style="width: 69%; border: 1px solid #99ffff;height: 20px;" placeholder="Search..." type="text" name="search_text" value="<?php if(isset($_POST['search_text']))echo $_POST['search_text']; ?>"/></span>
					<span>
                  ...
                  <div class="actions" style="float:right;padding-right:5px;">
                    <button class="button" type="button" onclick="window.location='add_guest.php';">
